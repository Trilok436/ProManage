...
import { deleteTasks, updateTaskStatus } from "../features/taskSlice";
import {
  Bug,
  CalendarIcon,
  GitCommit,
                  ...
                  filteredTasks.map((task) => {
                    ...
                    return (
                      <tr
                        key={task.id}
                        onClick={() => {
                          // We check both nested project object and direct projectId field
                          const pId = task.project?.id || task.projectId;
                          if (pId) {
                            navigate(
                              `/taskDetails?projectId=${pId}&taskId=${task.id}`,
                            );
                          } else {
                            toast.error(
                              "Could not find Project ID for this task",
                            );
                          }
                        }}
